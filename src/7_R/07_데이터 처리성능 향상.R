...
flights_df%>%
    arrange(desc(origin))
## datatable
flights[order(origin,-dest)] # 데이터테이블은 문자에 - 붙어도 색인화 가능.

#4. arr_delay열만 출력
flights_df[,"arr_delay"] # 벡터 형태로 출력
flights_df[,"arr_delay",drop=F] #데이터프레임 형태로 출력

flights[,arr_delay] # 벡터 형태로 출력
subset(flights,select=arr_delay) # 데이터프레임 형태로 출력
#5. year열부터 dep_time열까지 출력
colnames(flights_df)
flights_df[,1:4]
flights_df[,c('year':'dep_time')] # 불가능

flights[,year:dep_time] # 데이터 테이블 가능
...
flights_df[flights_df$month==6 & flights_df$origin=='JFK',c('arr_delay','dep_delay')]

flights[origin=='JFK'&month==6,.(arr_delay,dep_delay)]
#13. JFK 공항의 6월 운항 데이터 중 arr_delay열과 dep_delay열을 제외한 모든 열 출력
colnames(flights_df)
flights_df[flights_df$month==6& flights_df$origin=='JFK',c(-5,-7)]

flights[origin=='JFK'&month==6, -c('arr_delay','dep_delay')] #데이터 프레임 형태로 변경하고 출력
...
flights[carrier=='AA',.(mean_arr=mean(arr_delay),mean_dep=mean(dep_delay)),by=.(origin,dest,month)]
...
table(flights_df$depup0,flights_df$arrup0)

flights[,.N,by=.(dep_delay>0,arr_delay>0)]
#20. Origin==“JFK”에 대해 월별 최대 출발 지연 시간 출력(month로 정렬)
flights_JFK <- flights_df[flights_df$origin=='JFK',]
flights_JFK
summaryBy(dep_delay~month,flights_JFK,FUN=max)

flights[origin=='JFK',.(m=max(dep_delay)),keyby=.(month)]
